  ...
  methods: {
    ...
    init() {
      ...
      if (detail) {
        ...
        // 禁用检验单编号，检验日期，加工单编号
        this.headerFormRules['returnCode'][0].disabled = true;
    ...
    checkCount () {
      let values = this.mate.mateFormerHeader.values;
      // 定义订单数量/退货数量
      let [orderCount, returnCount] = [values.orderCount, values.returnCount]
    ...
    handleChange(val, name) {
      this.isDirty = true;
      const changeControl = {
        returnCode: (val) => this.handleCode(val),
        default: () => { return }
      }
    ...
    handleCode(val) {
      if (!val) return;
      SERVICES.RETURN_ORDER.checkCode(val).then(res =>{
        ...
        if (exist){
          ...
          this.mate.mateFormerHeader.values['returnCode'] = "";
        }
      })
    },
    /**
