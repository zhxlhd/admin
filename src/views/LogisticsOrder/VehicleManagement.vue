...
export default {
  ...
  methods: {
      ...
      if (ids.length > 0) {
        this.$confirm(
          ...
          this.$t("TITLE_TEXT.TIPS"),
          {
            ...
            SERVICES.VEHICLE_MANAGEMENT.deleteVehicleInfo(ids).then((res) => {
              if(res.status == 200) {
                this.$message({
                    type: "success",
    ...
    submitSelect () {
      ...
      const param = {
        'vehicleList' : this.multipleSelection,
        'registrationId' : this.selectedLines[0].id
      }
      SERVICES.VEHICLE_MANAGEMENT.createVehicleRecords(param).then(res => {
