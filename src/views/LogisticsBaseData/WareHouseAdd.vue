  ...
  data() {
    var isTel = (rule, value, callback) => {
      ...
      this.timerOut = setTimeout(() => {
        if(!/(^0\d{2,3}\-\d{7,8}$)|(^1[3|4|5|6|7|8][0-9]{9}$)/.test((value).trim())){
          return callback(new Error('请填入有效的电话(座机区号以“-”隔开)'));
        } else {
          callback();
        }
      }, 300);
    };
    var officeId = (rule, value, callback) => {
      if (!value) {
        return callback(new Error('此项不能为空'));
      }else{
        callback();
    ...
    return {
      ...
      headerFormRules: {
        // 仓库代码
        warehouseCode: [{ required: true, trigger: "blur", disabled: false, message:"此项不能为空"}],
        // 仓库名称
        warehouseName: [{ required: true, trigger: "blur", disabled: false, message:"此项不能为空"}],
        // 仓库地址
        warehouseAddress: [{ required: true, trigger: "blur", disabled: false, message:"此项不能为空"}],
        // 仓库联系人
        contact: [{ required: true, trigger: "blur", disabled: false, message:"此项不能为空"}],
        // 联系人电话
        phone: [{ required: true, trigger: "blur", disabled: false, validator: isTel}],
        // 所属机构
        officeId: [{ required: true, trigger: "change", disabled: false, validator: officeId}],
      },
      headerFormEventConfig: {
        warehouseCode: { blur: true }
      }
    };
  },
  created() {
    ...
    if(mateData && mateData.id && mateData.id !== '') {
      ...
      // 仓库代码 修改的时候不允许修改，以保证仓库代码唯一
      this.headerFormRules.warehouseCode[0].disabled = true;
    }
  },
  methods: {
    /** 头部按钮事件
    ...
    handleEvents(event, val, name) {
      const eventControl = {
        blur: (val, name) => this.handleWarehouseCodeBlur(val, name),
        default: () => { return }
      }
      return (eventControl[event] || eventControl.default)(val, name);
    },
    /** blur 事件 Code 仓库代码 校验是否唯一
     * @param {Any} val 控件的值
     * @param {String} name 控件的 name
     * @return void
     */
    handleWarehouseCodeBlur(val, name) {
      if (val !== '') {
        SERVICES.LOGISTICS_BASE_DATA.checkWarehouseCode(val.trim()).then(res => {
          if(!res.data.status){
            this.mate.mateFormerHeader.values[name] = '';
            this.$message({
              type: "warning",
              message: "此技术仓库代码已存在，请重新录入！"
            });
          }
        });
      }
      ...
      SERVICES.LOGISTICS_BASE_DATA.getOrgList().then((res) =>{
        if(res.data.length>0){
          ...
          if(data.length){
            ...
            // 所属机构保存到表单配置中
            this.headForm['officeId'].options = orgOptions;
          }
        }
        });
    },
     ...
     */
    backToList () {
      this.$root.monitor({ url: LOGISTIC_BASIC_DATA_API.WAREHOUSE_LIST });
    }
  }
};
</script>
