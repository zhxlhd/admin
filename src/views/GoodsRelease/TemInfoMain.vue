...
<template>
  ...
  <el-dialog title="发布行业资讯" :visible.sync="dialogFormVisible" width="30%">
    <el-form>
...
export default {
  ...
  methods: {
    ...
    DialogClickOK() {
      ...
      this.$root.ajaxData(`/electro/el/news/publish/${val}`, rowt, function(data){
        vm.$root.monitor({ url: '/electro/el/news/all-list', method: 'GET' })
      }, 'PUT')
    },
   ...
   handleButton (btn) {
      ...
      const rowt = this.multipleSelection.map((r) => r.id)
      const newsStatus = this.multipleSelection.map((q) => q.state)
      const newsState = ''
      // 行业资讯列表删除
      if (btn.action === 'delNews') {
        // 新建、废弃状态可以删除
        var isDelNews = false
        for(let i=0;i<newsStatus.length;i++) {
          this.newsState = newsStatus[i]
          if (this.newsState === '1' || this.newsState === '2' || this.newsState === '4') {
            isDelNews = true
          }
        }
        if (isDelNews === true) {
          ...
          this.$confirm('此操作将此资讯数据删除, 是否继续?', '提示', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning'
          }).then(() => {
            this.$root.ajaxData('/electro/el/news/delete',rowt, function (data) {
      ...
      if (btn.action === 'submitAudit') {
        ...
        for(let i=0;i<newsStatus.length;i++) {
          this.newsState = newsStatus[i]
          if (this.newsState === '0' || this.newsState === '3') {
            isSubmitAudit = true
          }
        }
        if (isSubmitAudit === true) {
          this.$root.ajaxData('/electro/el/news/handle/1' , rowt, function () {
            vm.$root.monitor({ url, method: 'GET' })
          }, 'PUT')
          return
        }
      ...
      if (btn.action === 'updateAudit') {
        ...
        this.$root.ajaxData('/electro/el/news/handle/2', rowt, function (data) {
          vm.$root.monitor({ url, method: 'GET' })
        }, 'PUT')
        return
      }
      ...
      if (btn.action === 'rejectAudit') {
        ...
        this.$root.ajaxData('/electro/el/news/handle/3', rowt, function (data) {
          vm.$root.monitor({ url, method: 'GET' })
        }, 'PUT')
        return
      }
      ...
      // 行业资讯发布
      if (btn.action === 'newsIssue') {
        var isNewsIssue = false
        for(let i=0;i<newsStatus.length;i++) {
          this.newsState = newsStatus[i]
          if (this.newsState !== '2') {
            isNewsIssue = true
      ...
      // 行业资讯废弃
      if (btn.action === 'newsDiscard') {
        var isNewsDiscard = false
        for(let i=0;i<newsStatus.length;i++) {
          this.newsState = newsStatus[i]
          if (this.newsState !== '4') {
            isNewsDiscard = true
        ...
        if (isNewsDiscard === true) {
          ...
          this.$confirm('是否确认将已发布的信息废弃？','提示',{
              confirmButtonText: '确定',
              cancelButtonText: '取消',
              type: 'warning'
          }).then(() => {
              this.$root.ajaxData('/electro/el/news/handle/5', rowt, function (data) {
                  vm.$root.monitor({ url, method: 'GET' })
              }, 'PUT')
              this.$message({
                  message: '删除成功',
    ...
    getData (flag) {
      let url = '/electro/el/news/search-list'; // 分页、搜索查询接口
      let vm = this;
      // 查询
