<!-- created by hezhiqiang on 2020/05/13 -->
<!-- 入库 -->
<template>
  <div>
    <!-- headTable Start -->
...
export default {
  ...
  methods: {
    ...
    getListData () {
      ...
      SERVICES.STOCK_LIST.getList(param).then(res => {
    ...
    handleButtons (btn, rowId, selection) {
      ...
      const btnEvents = {
        add: (btn) => this.addStocck(btn),                     // 弹窗新增
        default: () => { return }
      }
     ...
     */
    getDetailList () {
      SERVICES.STOCK_LIST.getDetailList(this.headRowId).then((res) => {
