...
export default {
  ...
  data() {
    return {
      ...
      lineFormRules : {
        // 出库单价
        deliveryUnitPrice: [{ required: true, message: this.$t('FORM_VALIDATION_INFO.IS_REQUIRED'), trigger: "blur"}],
        // 商品编码
      ...
      lineFromEventConfig: {
        ...
        // 出库数量
        deliveryQuantity: {input: true},
        // 出库单价
        deliveryUnitPrice: {input: true},
      },
    };
  },
  created() {
  ...
  methods: {
    ...
    handleButtons(btn, row) {
      ...
      const btnEvents = {
        ...
        batchOutbound: () => this.handleBatchOutbound(),    // 批量出库
        // 默认
        default: () => { return }
      }
    ...
    handleBatchOutbound () {
      let param = {};
      param.warehouse = this.mate.mateFormerHeader.values.warehouse;
      SERVICES.ISSUE_LIST.batchOutbound(param).then(res => {
        if (res.status == 200) {
    ...
    handleSubmit() {
      ...
      this.$refs['headForm'].$refs["form"].validate(valid => {
        if (valid) {
          ...
          mate = _this.saveData();
          if (!mate) return;
          SERVICES.ISSUE_LIST.save(mate).then((res) =>{
    ...
    handleInput(val, name) {
      const inputControl = {
        ...
        // 出库数量
        deliveryQuantity: (val, name) => this.handleDeliveryQuantity(val),
        // 出库单价
        deliveryUnitPrice: (val) => this.handleDeliveryUnitPrice(val),
        default: () => { return }
      }
    ...
    /**
     * 出库数量*出库单价=获取出库金额
     * @param {String} val 出库数量
     * @param {String} 出库单价
     */
    handleDeliveryQuantity(val){
      if (val && this.mate.mateFormerLine.values.deliveryUnitPrice) {
        let data = (val * this.mate.mateFormerLine.values.deliveryUnitPrice).toFixed(2)
        this.$set(this.mate.mateFormerLine.values,"issueAmount",data)
      }
    },
    /**
     * 出库数量*出库单价=获取出库金额
     * @param {String} val 出库单价
     * @param {String} 出库数量
     */
    handleDeliveryUnitPrice (val){
      if (val && this.mate.mateFormerLine.values.deliveryQuantity) {
        let data = (val * this.mate.mateFormerLine.values.deliveryQuantity).toFixed(2)
        this.$set(this.mate.mateFormerLine.values,"issueAmount",data)
      }
    },
    /** 
    ...
    changeCode(val) {
      ...
      SERVICES.ISSUE_LIST.getArerageCost(val).then(res => {
        if (res.status === 200) {
          let data = res.data.data.list;
          let price = '';
          if (data.length) {
            data.forEach(item => {
              price = item.weightedAverageCost
            })
            this.$set(this.mate.mateFormerLine.values,'deliveryUnitPrice',price)
          }else {
            // this.$message({
            //   type: 'warning',
            //   message: '该商品编码提供加权平均单价，请填写'
            // });
            let deliveryUnitPrice = this.mate.mateFormerLine.fields.find(item => item.name === 'deliveryUnitPrice');
            this.mate.mateFormerLine.values.deliveryUnitPrice = '';
            deliveryUnitPrice.disabled = false;
          }
        }
      })
    },
    ...
    resetDialogForm (values){
      ...
      values['deliveryQuantity'] = "";           // 出库数量
      values['deliveryUnitPrice'] = "";          // 出库单价
      values['issueAmount'] = "";                // 出库金额
      values['signedBy'] = "";                   // 签收人
      values['signedInQuantity'] = "";           // 签收数量
      values['signeDate'] = "";                  // 签收日期
      values['remark'] = "";                     // 备注
    },
    /**
     * 递归自动生成序号 10/20/30....
