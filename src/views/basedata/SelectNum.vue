...
<div>
      ...
      <el-row>
        <el-col :span="24">
          <el-table highlight-current-row ref="table" :data="mate.rows" border @selection-change="handleSelectionChange" @sort-change="sortChange" style="width: 100%;">
              ...
              <el-table-column :label="object['pickQty'].label" :prop="object['pickQty'].name" :width="object['pickQty'].width">
                <template slot-scope="scope">
                  <el-input size="mini" v-model="scope.row[object['pickQty'].name]"></el-input>
                </template>
              </el-table-column>
  ...
  <el-button type="primary" class="buttonFinished" @click="submit">{{$t('selected')}}</el-button>
</div>
</template>
...
export default {
  ...
  methods: {
    ...
    submit () {
      var vm = this
      // this.multipleSelection表示在table中选中的数据
      if (this.multipleSelection.length === 0) {
        this.$message({
          message: this.$t('selectedHintMsg'),
          type: 'warning'
        })
        return
      }
      if (this.mate.rows[0].pickQty > this.mate.rows[0].unpickQty) {
        this.$message({
          message: this.$t('Inventory shortage'),
          type: 'warning'
        })
        return
      }
      this.$root.ajaxData('/outbound/pick/lines/toPick', this.multipleSelection, function () {
        // vm.$root.ajaxData('/outbound/bill/headers/all/list', {}, function () {}, 'GET')
