...
export default {
  data() {
    ...
    return {
      ...
      labelPosition: "right",
      rules: {
        officeId: [{ required: true, message: "该输入项为必输项" }],
        supplierId: [
  ...
  components: {
    PurchaseOrderKtable: function index(resolve) {
      require([
        "../../views/PurchasingManagement/PurchaseOrderKtable.vue"
      ], resolve);
    },
  ...
  methods: {
    ...
    getCustomer() {
      var vm = this;
      this.$root.ajaxData(
        "/companys/companyInfo/4?status=1&auditStatus=1&nopaging=1&dataOnly=true",
        {},
        function(data) {
    ...
    add() {
      ...
      if (this.soTableFlag) {
        if (
          this.mate.mateFormerHeader.values.officeId === null ||
          this.mate.mateFormerHeader.values.officeId === ""
        ) {
          vm.$message({
            message: "请选择机构",
            type: "warning"
        ...
        this.$root.ajaxData(
          "/purchase/goodsPricing/line/getSoLines?customerId=" +
            this.mate.mateFormerHeader.values.customerId +
            "&officeId=" +
            this.mate.mateFormerHeader.values.officeId,
          {},
            ...
            if (this.poTableFlag) {
              if (
                this.mate.mateList.rows[j].cusPo ===
                  this.confrimRows[index].cusPo &&
                this.mate.mateList.rows[j].cusPoLineNum ===
                  this.confrimRows[index].cusPoLineNum
              ) {
            ...
            if (this.soTableFlag) {
              if (
                this.mate.mateList.rows[j].soNum ===
                  this.confrimRows[index].soNum &&
                this.mate.mateList.rows[j].soLineNum ===
                  this.confrimRows[index].soLineNum
              ) {
                this.confrimRows.splice(index, 1);
                break;
              }
    ...
    async handleSubmit() {
      ...
      for (var k = 0; k < this.list.rows.length; k++) {
        ...
        if (this.poTableFlag) {
          checkStatusFlag = await this.$http({
            url:
              API_GATEWAY +
              "/api/purchase/asn/getStateByNo?poNo=" +
              this.list.rows[k].poNo +
              "&polineNo=" +
              this.list.rows[k].polineNo +
              "&pllShipmentNum=" +
              this.list.rows[k].pllShipmentNum +
              "&lineNum=" +
              this.list.rows[k].lineNum,
            method: "GET",
            headers: {
      ...
      this.form.values.deliveryTime = DateFormat(
        this.form.values.deliveryTime,
        "yyyy-MM-dd"
      );
      this.form.values.arrivalTime = DateFormat(
        this.form.values.arrivalTime,
        "yyyy-MM-dd"
      );
    ...
    backward() {
      this.$root.monitor({ url: "/purchase/asn/all/list?billType=0" });
    },
    lineSelected(val) {
