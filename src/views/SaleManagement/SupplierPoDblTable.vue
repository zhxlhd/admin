...
vm.companyName="";
    ...
    rowClick (row) {
      ...
      this.$root.ajaxData('/supplier-portal/po/line/list?poNum=' + id, {}, function (data) {
    ...
    closePrintDialog () {
      ...
      let rowCustomerName = this.rowData.customerName;
      let value = this.printValue;
      let ebsPoHeaderId = this.rowData.ebsPoHeaderId;
      let poNum = '3110000129';
      console.log(value)
      url = API_GATEWAY + '/api/supplier-portal/po/print/pd_good_pricing?format=pdf&poNum='+ poNum
      var vm = this
      vm.$http({
          url: url,
          method: 'GET',
    ...
    handleButton (btn) {
      ...
      if (btn.action === 'exportPoHeaders') {
        this.$http({
                  url: API_GATEWAY + '/api/purchase/goods/pricing/export/po/headers?customerName='+this.companyName+'&goodsName='+this.opNum+'&state='+this.poSelect,
                  method: 'GET',
                  responseType: 'arraybuffer',
      ...
      if (btn.action === 'query') {
        ...
        this.$root.ajaxData(`/supplier-portal/po/head/list/query?customerName=${this.companyName}&goodsName=${this.opNum}&state=${this.poSelect}`, {}, function (data) {
      ...
      var poNum = this.multipleSelection[0].goodsName
      var orgId = this.multipleSelection[0].orgId
      ...
      if (btn.action === 'allReceive') {
        // if (state === '0' || state === '6') {
        if(1==1){
          this.$root.ajaxData(`/supplier-portal/po/head/update?poNum=${poNum}&state=3&orgId=${orgId}`, {}, function () {
      ...
      if (btn.action === 'allReject') {
        // if (state === '0' || state === '6') {
          if(1==1){
          this.$root.ajaxData(`/supplier-portal/po/head/update?poNum=${poNum}&state=4&orgId=${orgId}`, {}, function () {
            vm.$root.monitor({ url: '/purchase/cusPo/getAllPo/new' })
          }, 'PUT')
          return
        } else {
          this.message('warning', '不是新建、已变更状态')
      ...
      if (btn.action === 'save') {
        if (state === '0' || state === '6') {
          ...
          this.$root.ajaxData(`/supplier-portal/po/head/update?poNum=${poNum}&state=5&orgId=${orgId}`, mate, function () {
            vm.$root.monitor({ url: '/purchase/cusPo/getAllPo/new' })
          }, 'PUT')
          return
        } else {
    ...
    getData (flag) {
      ...
      var vm = this
      url += '?customerName='+this.companyName+'&goodsName='+this.opNum+'&state='+this.poSelect+'&' + 'search=' + encodeURIComponent(JSON.stringify(this.searchValues)) + '&' + (this.toUriParams(this.sortValues)) + '&pageNum=' + this.page + '&pageSize=' + this.pageSize
      param = null
