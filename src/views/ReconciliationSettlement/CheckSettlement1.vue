<template>
  <div class="CheckSettlement1">
    <div class="headQuery">
      <check-s-form1 class="form" :form="mate.queryHeader" :showFlag="!!mate.matePaymentList.selectedRows.length" :saleCheck="saleChecked" @changeThree="changeThree" @clickCreate="clickCreate"></check-s-form1>
    ...
    <div class="client">
      ...
      <div class="client-btn">
        ...
        <el-button type="primary" plain size="mini" @click="adjustPrice" :disabled="payDataSelectList.length!==1" v-show="mate.matePaymentList.btns[3].adjustTax">调价</el-button>
    ...
    <!-- <div class="supplier" v-show="saleChecked">
      <h1>销售方待对账清单</h1>
      <div class="client-btn">
        <el-button size="mini" @click="match">新增</el-button>
        <el-button size="mini" @click="supplierDelete" :disabled="!this.reDateSelectList.length">删除</el-button>
      </div>
      <ag-grid-vue style="height: 300px;"
                  class="ag-theme-balham"
                  :gridOptions="gridOptions"
                  :columnDefs="mate.mateReceiveList.columns"
                  :rowData="mate.mateReceiveList.selectedRows"
    ...
    <!--新增供应商对账单dialog-->
    <check-dialog-supplier :mate="mate.mateReceiveList" :dialogSupplier="dialogSupplier" :fourCode="fourCode" @supplierClose="supplierClose" @supplierQuery="supplierQuery" @actionSelectedRe="actionSelectedRe"></check-dialog-supplier>
...
<script>
  ...
  import Vue from 'vue'
  import CheckSForm1 from './CheckSForm1'
  ...
  export default {
    ...
    methods: {
      ...
      priceSubmit () {
        ...
        let maxLength
        this.adjustPriceDialog = false
        let payData = this.payDataSelectList[0]
        let data = JSON.parse(JSON.stringify(payData))
        data.qty = this.adjustPriceM.number
        ...
        data.parentSourceId = data.sourceId
        this.mate.matePaymentList.selectedRows.splice(data.lineNo, 0, data)
        this.payDataSelectList[0].isChange = true
        this.sortPaySelect()
        this.clearNumber()
        this.payDataSelectList = []
      },
      ...
      adjustPrice () {
        if (this.payDataSelectList[0].isChange) {
          this.$message({
            message: '该条数据已调价，请选择其他数据进行调价',
            type: 'warning'
          })
          return
        }
        this.adjustPriceDialog = true
        this.adjustPriceM.number = this.payDataSelectList[0].qty
      ...
      selectionChanged (val) {
        this.reDateSelectList = this.gridOptions.api.getSelectedRows()
        this.caclRe()
      },
      // table选中的数据应付
      ...
      // 确定选择对账方式
      getCheckResult () {
        this.saleChecked = this.saleCheck
        this.dialogVisible = false
        this.$emit('saleCheckedMethod', this.saleChecked)
      },
      // 关闭选择对账方式dialog
      handleClose () {
        this.dialogVisible = false
      ...
      clickAll () {
        ...
        if(this.isMutiAddress){
          if (queryHeader.ouName == '' || queryHeader.customerName == '' || queryHeader.currencyCode == '') {
            this.$message({
              message: '请先填写购方业务实体、购方公司',
              type: 'warning'
            })
            return false
          }
          this.fourCode = {
            ...
            currency: queryHeader.currencyCode
          }
          return true
        } else {
          if (queryHeader.ouName == '' || queryHeader.customerName == '' || queryHeader.currencyCode == '' || queryHeader.address == '') {
          this.$message({
            message: '请先填写购方业务实体、购方公司、地点和币种',
            type: 'warning'
          })
          return false
        }
      ...
      clientQuery (val) {
        ...
        this.$root.ajaxData('/purchase/recon/pre/query/payment', params, function (data) {
      ...
      supplierClose () {
        ...
        this.mate.mateReceiveList.rows = []
      },
      // 查询供应商待对账清单
      supplierQuery (val) {
        var vm = this
        ...
        let arrId = []
        params.arList = val
        this.mate.mateReceiveList.selectedRows.length && this.mate.mateReceiveList.selectedRows.map(i => {
          arrId.push(i.id)
        })
        params.selectedIdList = arrId
        this.$root.ajaxData('/purchase/recon/pre/match/receive', params, function (data) {
          for (let i = 0; i < data.rows.length; i++) {
            data.rows[i].taxMoney = data.rows[i].moneyAmount - data.rows[i].noratAmount
          }
          vm.mate.mateReceiveList.rows = data.rows
        }, 'POST')
      },
      // 从子组件过来
      ...
      caclRe () {
        ...
        this.reDateSelectList.forEach(i => {
          if (typeof i.id === 'string') {
            qtyTotal += Number(i.quantityOrdered)
          }
          money += Number(i.amount)
        })
        this.mate.queryHeader.qytMount = qtyTotal
        this.mate.queryHeader.amount = (Math.round(money * 100) / 100).toFixed(2)
        this.mate.queryHeader.differOfQty = this.mate.queryHeader.qty - this.mate.queryHeader.qytMount
        this.mate.queryHeader.differOfMoney = (Math.round((this.mate.queryHeader.moneyAmount - this.mate.queryHeader.amount) * 100) / 100).toFixed(2)
      },
      judgeAddress () {
      ...
      clickCreate () {
        ...
        if (this.saleChecked) {
          ...
          for (var i = 0; i < this.payDataSelectList.length - 1; i++) {
            for (var j = i+1; j < this.payDataSelectList.length; i++) {
              if (this.payDataSelectList[i].currency !== this.payDataSelectList[j].currency) {
                this.$message({
                  message: '不同币种的待对账清单无法生成结算单',
                  type: 'warning'
                })
                return
              }
        ...
        if (this.saleChecked) {
          ...
          this.methodsUrl('/purchase/ap/create/settle', params)
        }
      },
