<template>
  <div>
    <el-row>
      <el-col :span="24">
        <Boxer :mate="mate.mateList1"  :mateSearch="mate.mateSearch" :hasSelected="hasSelected" :btnDisable="btnDisable" @action="handleButton" ref="Boxer" :msgBoxButton="msgBoxButton">
        ...
        <div slot="main">
          ...
          <el-row type="flex" justify="center" class="page" ><small></small>
          <el-col :span="12">
            <el-pagination
    ...
    <!--  弹出框，用于查看用户详情信息 -->
    <el-dialog title="客户详情" :visible.sync="customerDetail" width="80%">
        ...
        <el-form size='mini' :model="mate.mateFormer.values" :rules="rules" ref="ruleForm" :label-width="labelWidth" class="elForm" :label-position="labelPosition">
          <div class="clientAddMsg">
            <div v-for="(field,index) of mate.mateFormer.fields" class="clientAddMsgDiv" :key="index">
              ...
              <el-col :sm="12" :md="8" :lg="8">
                <el-form-item class="el-form-item" :label="field.label" :prop="field.name" v-if="field.holder == 'textCustomer'">
                  <el-input disabled v-model="mate.mateFormer.values[field.name]"></el-input>
                </el-form-item>
              </el-col>
              ...
              <el-form-item class="el-form-item" :label="field.label" :prop="field.name" v-if="field.holder == 'switch'">
                <el-switch :active-text="field.on" :inactive-text="field.off" v-model="mate.mateFormer.values[field.name]"></el-switch>
              </el-form-item>
              </el-col>
              <el-col :sm="12" :md="8" :lg="8">
              <el-form-item class="el-form-item" :label="field.label" :prop="field.name" v-if="field.holder == 'select'">
                <el-select clearable disabled v-model="mate.mateFormer.values[field.name]" :placeholder="field.placeholder">
                  <el-option :label="item.label" :key="item.value" :value="item.value" v-for="item of field.options"></el-option>
                </el-select>
              </el-form-item>
              </el-col>
              <el-col :sm="12" :md="8" :lg="8">
              ...
              <el-col :sm="12" :md="8" :lg="8">
                <el-form-item  :label="field.label" :prop="field.name" v-if="field.holder === 'cascaderC'">
                  <el-input disabled :options="optionsValue" change-on-select v-model="mate.mateFormer.values[field.name]"></el-input>
                </el-form-item>
              </el-col>
            </div>
          </div>
      ...
      <el-collapse v-model="activeName" accordion @change="handleCollapse" class="collapse">
        <el-collapse-item title="客户地点信息" name="1">
          <el-tabs type="card" v-model="activeName1" @tab-click="handleClick">
            ...
            <el-tab-pane label="销售信息" :show="show" name="2">
              <CTable :mate= "customerData.AddSellMsg" @rowClick="rowClickTable" :tableData="tableData2"></CTable>
            </el-tab-pane>
...
export default {
  ...
  data () {
    return {
      pageSize: 15,
      disabledFlag: true,          // 省市区都不能改动
      optionsValue: [],                // 级联的行业细分类
  ...
  methods: {
    query () {
      ...
      this.$root.ajaxData(url, mate, function (data) {
        vm.mate.mateList1.rows = data.mate.rows;
        vm.mate.mateList1.total = data.mate.total;
      }, 'POST')
    },
    showSearchPopover () {
      ...
      if (activeName === '1') {
        if (this.tableData1) {
          this.$root.ajaxData('/addresses/all/' + this.idCustomer, {}, function (data) {
            vm.tableData1 = data
            // vm.getDataAccounts(vm.idCustomer)
            // vm.getDataContacts(vm.idCustomer)
          }, 'GET')
        }
    ...
    handleClick (tab, event) {
      var vm = this
      if (tab.name === '2') {
        this.$root.ajaxData('/sells/all/' + this.idCustomer, {}, function (data) {
          vm.tableData2 = data
        }, 'GET')
      } else if (tab.name === '3') {
    ...
    DialogClickOK () {
      ...
      this.$root.ajaxData('/companys/auditStatus/' + this.valueStatus + '?ids=' + rowt.id, {}, function (data) {
        vm.$root.ajaxData('/companys/companyInfo/4', {}, function (data) {           // 刷新
    ...
    getData (flag) {
      let url = '/companys/customer/list'
      let param = {
