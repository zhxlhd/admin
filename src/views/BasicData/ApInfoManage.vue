<!--应付会计管理  fengjing  20200609  主要是系统管理员操作-->
<template>
	<div>
		<KTableList
				:mate="mate"
  ...
  components: {
	...
	data(){
      ...
      queryParam: {
        ...
        pageSize: 10
			...
			formRules: {
				// 事业部名
				ouName: [{ required: true, trigger: "blur", disabled: false, message:"此项不能为空"}],
				// 应付会计名
				apName: [{ required: true, trigger: "blur", disabled: false, message:"此项不能为空"}],
				// 应付会计邮箱
				apEmail: [{required: true,trigger: "blur",disabled: false, message:"此项不能为空"}]
			},
			url: {
    ...
    handleButtons (btn, rowId, selection) {
      ...
      const btnEvents = {
        ...
        exportDetail: (btn) => this.handleExport(btn),                      // 导出
        default: () => { return }
      }
     ...
     */
    handleTemplate(btn){
      let [url, methods, fileName, type, data] = [ btn.url, 'POST', '应付会计导入模板', 'xlsx', {}];
     ...
     */
    handleExport(btn){
			let [url, methods, fileName, type, data] = [ btn.url, 'POST', '应付会计清单', 'xlsx', this.mate.mateSearch.values];
      SERVICES.BASE.downTemplateExcel(url, methods, fileName, type, data);
    },
		/**
		 * 弹出框取消按钮
					...
					if(this.isEdit){
						putAction(`${this.url.updateUrl}${params.id}`, params).then(res=>{
							if(res.data.statusCode == 500){
								...
								_this.$root.monitor({ url: '/common/toList?listName=ap_info_list&listFormName=ap_info_form&path=' })
								_this.dialogVisible = false
							}
						})
					} else {
