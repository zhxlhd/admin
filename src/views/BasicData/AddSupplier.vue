<template>
  <div>
    ...
    <div v-if="mateValuesId">
      ...
      <el-tabs type="card" >
        ...
        <el-tab-pane label="采购信息">
          <CTable @rowClick="rowClick" :mate="customerData.AddPoMsg" :tableData="tableData2" :show="!show" @action="handleUpDataAddr" @actionDel="handleDelAddr">
          </CTable>
        </el-tab-pane>
    ...
    <!--供应商地点信息专用-->
    <el-dialog v-if="mateValuesId" title="新增" :visible.sync="dialogAddrMsg" width="80%">
      <el-row>
        <el-form size="mini" :model="addrSellerFani.newMainMsg" :rules="newMainMsg.rules" ref="ruleAddrForm" :label-width="labelWidth" label-position="right">
          <el-tabs v-model="activeName2" type="card">
            <el-tab-pane label="主要信息" name="first">
              ...
              <el-col :sm="12" :md="8" :lg="6">
                ...
                <el-form-item :label="customerData.newMainMsg.fields[3].label" :prop="customerData.newMainMsg.fields[3].name">
                  <el-input v-model="addrSellerFani.newMainMsg.name" :disabled="showEdit"></el-input>
                </el-form-item>
              </el-col>
              <el-col :sm="12" :md="8" :lg="6">
                <el-form-item :label="customerData.newMainMsg.fields[7].label" :prop="customerData.newMainMsg.fields[7].name">
                  <el-input v-model="addrSellerFani.newMainMsg.address" :disabled="showEdit"></el-input>
                </el-form-item>
              </el-col>
            ...
            <el-tab-pane label="采购信息" name="second">
              ...
              <el-col :sm="12" :md="8" :lg="6">
                <el-form-item :label="customerData.newPoMsg.fields[0].label" :prop="customerData.newPoMsg.fields[0].name">
                  <el-select clearable v-model="addrSellerFani.newPoMsg['canPurchase']">
                    <el-option :label="item.label" :value="item.value" v-for="(item,index) of customerData.newPoMsg.fields[0].options" :key="index"></el-option>
                  </el-select>
                </el-form-item>
              </el-col>

              <el-col :sm="12" :md="8" :lg="6">
                <el-form-item :label="customerData.newPoMsg.fields[1].label" :prop="customerData.newPoMsg.fields[1].name">
                  <el-select clearable v-model="addrSellerFani.newPoMsg['canPayment']">
                    <el-option :label="item.label" :value="item.value" v-for="(item,index) of customerData.newPoMsg.fields[1].options" :key="index"></el-option>
                  </el-select>
                </el-form-item>
              </el-col>

              <el-col :sm="12" :md="8" :lg="6">
                <el-form-item :label="customerData.newPoMsg.fields[2].label" :prop="customerData.newPoMsg.fields[2].name">
                  <el-select clearable v-model="addrSellerFani.newPoMsg['enquiry']">
                    <el-option :label="item.label" :value="item.value" v-for="(item,index) of customerData.newPoMsg.fields[2].options" :key="index"></el-option>
                  </el-select>
                </el-form-item>
              </el-col>
      ...
      <div class="submitCancel">
        ...
        <el-button size="mini" @click="dialogAddrMsg = false, backHandleAddress('ruleAddrForm')">取 消</el-button>
      </div>
    </el-dialog>

    <!--银行专用-->
    <el-dialog v-if="mateValuesId" title="新增" :visible.sync="dialogBankMsg" width="80%">
      <el-row>
      <el-form size="mini" :model="newBankMsg.values" :rules="newBankMsg.rules" ref="ruleBankForm" :label-width="labelWidth" :label-position="labelPosition">
        
        <el-col :sm="12" :md="8" :lg="6">
            <el-form-item :label="bankMsgObject['addressId'].label" :prop="bankMsgObject['addressId'].name">
              <el-select clearable @visible-change="visibleChangeAddr" v-model="newBankMsg.values[bankMsgObject['addressId'].name]">
                <el-option :label="item.label" :value="item.value" v-for="(item,index) of bankMsgObject['addressId'].options" :key="index"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
      ...
      <div class="submitCancel">
        ...
        <el-button size="mini" @click="dialogBankMsg = false, backHandleBank('ruleBankForm')">取 消</el-button>
      </div>
    </el-dialog>

    <!--联系人专用-->
    <el-dialog v-if="mateValuesId" title="新增" :visible.sync="dialogCoutomer" width="80%">
      <el-row>
        <el-form size="mini" :model="newCoutomerMsg.values" :rules="newCoutomerMsg.rules" ref="ruleCoutomerForm" :label-width="labelWidth" :label-position="labelPosition">
    ...
    <!--附件专用-->
    <el-dialog v-if="mateValuesId" title="新增" :visible.sync="dialogFile" width="80%">
      <el-row>
      <el-form size="mini" :model="newFile.values" :rules="newFile.rules" ref="ruleFileForm" :label-width="labelWidth" :label-position="labelPosition">
        <el-col :sm="12" :md="8" :lg="6">
...
export default{
  ...
  data () {
    return {
      ...
      addrSellerFani: { 'newMainMsg': {}, 'newPoMsg': {}, 'newFaniMsg': {}},
      activeName2: 'first',
      dialogAddrMsg: false,
      newBankMsg: {},
      showEdit: false,
  ...
  created () {
    ...
    this.addrSellerFani.newPoMsg = Object.assign({}, this.customerData.newPoMsg.values)
    ...
    this.newCoutomerMsg = this.customerData.newCoutomerMsg;
    this.newFile = this.customerData.newFile
    this.newCoutomerMsg.fields.map((i) => { this.coutomerObject[i.name] = i });
    this.mateValuesId = this.mate.values.id
    this.coutomerObject['addressId'].options = [...this.coutomerObject['addressId'].options];
    this.coutomerObject['sex'].options = [...this.coutomerObject['sex'].options];
    this.coutomerObject['sysAccountName'].options = [...this.coutomerObject['sysAccountName'].options];
    this.bankMsgObject['flagVatUsed'].options = [...this.bankMsgObject['flagVatUsed'].options];
    this.bankMsgObject['status'].options = [...this.bankMsgObject['status'].options];
    if (this.mateValuesId) {
      ...
      this.$root.ajaxData('/addresses/all/' + this.mateValuesId, {}, function (data) {
        ...
        vm.bankMsgObject['addressId'].options = [...options];
        vm.coutomerObject['addressId'].options = [...option];
      }, 'GET')
      // 获取到销售信息和财务信息
      this.getDataSells(vm)
      this.getDataFinances(vm)
      // 获取到地点信息的主要信息
      this.$root.ajaxData('/addresses/all/' + vm.mateValuesId, {}, function (data) {
        vm.tableData1 = data;
    ...
    this.distinguishIndustry();  // 通过行业区分,显示隐藏字段
  },
  methods: {
    /**
     * 通过行业区分,显示隐藏字段
    ...
    getDataSells (vm) {
      vm.$root.ajaxData('/po/all/' + vm.mateValuesId, {}, function (data) {
        vm.tableData2 = data
      }, 'GET')
    },
    getDataFinances (vm) {
    ...
    handleUpDataAddr (row) {
      ...
      this.$root.ajaxData('/addresses/getAll/' + row.id, {}, function (data) {
        vm.showEdit = true
        vm.$refs.CountrySelect.$refs['multiselect'].value = { 'nameZH': data.newMainMsg.country }
        vm.addrSellerFani.newMainMsg['province'] = data.newMainMsg.province;
        vm.initprovselectedvalue = data.newMainMsg.province
        vm.initcityselectedvalue = data.newMainMsg.city
        ...
        vm.cityAddrs = getAddrsArrayById(vm.initprovselectedvalue)
        vm.regionAddrs = getAddrsArrayById(vm.initcityselectedvalue)
        vm.initregionselectedvalue = data.newMainMsg.county
    ...
    handleSuccess (response, file) {
      ...
      this.newFile.values.fileSize = file.size + 'KB';
    },
    handlePreview () {
    ...
    handleUpDataBank (row) {
      ...
      this.newBankMsg.values = row
    },
    handleUpDataCustomer (row) {
      this.title = '修改'
      this.dialogCoutomer = true
      this.newCoutomerMsg.values = row
    },
    ...
    DialogClickOKAddress () {
      ...
      this.$refs.ruleAddrForm.resetFields()
      this.addrSellerFani = { 'newMainMsg': {}, 'newPoMsg': {}, 'newFaniMsg': {}}
    ...
    DialogCustomerClickOK (ruleForm) {
      ...
      var idNow = this.newCoutomerMsg.values.id
      var addressIdNow = this.newCoutomerMsg.values.addressId
      ...
      this.$refs.ruleCoutomerForm.validate((valid) => {
        if (valid) {
          if (idNow) {
            // 地点修改
            vm.$root.ajaxData('/contacts/' + addressIdNow, this.values, function () {
              vm.getDataContacts(addressIdNow)
    ...
    visibleChangeAddr (flag) {
      ...
      if (flag) {
        this.$root.ajaxData('/addresses/all/' + this.mateValuesId, {}, function (data) {
          ...
          // vm.coutomerObject['addressId'].options = Object.assign({}, options)
          vm.bankMsgObject['addressId'].options = [...options];
          vm.coutomerObject['addressId'].options = [...options];
        }, 'GET')
      }
    },
    makeBankInfo () {
