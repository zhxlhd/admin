...
<template>
  <div>
    <k-table-list :mate="mate" ref="listTable" :mateSearch="mate.mateSearch" :isIndex="isIndex" @initOperations="initOperations" @rowClick="getRowData" @action="handleActions" @btnClick="handleButtons"></k-table-list>
...
export default {
  ...
  data(){
    return {
      ...
      uploadAttachmentParams: { rowId:'', serviceModel:'LOGISTICS_BASE_DATA', getMethod:'getAttachMentListWithRegistrationConsignee', uploadMethod:'uploadAttachMent', deleteMethod:'deleteAttachMent', busType:'registration_consignee' } // 上传附件属性配置
    }
  },
  props: {
    mate: Object
  ...
  methods: {
    ...
    enableOrInvalid(act, row) {
      ...
      if (multipleSelection.length !== 1) {
        ...
        SERVICES.LOGISTICS_BASE_DATA.loseEffectWithConsignee(multipleSelection[0].id, multipleSelection[0].status).then(res => {
          if(res.status == 200) {
            this.$message({
                type: "success",
                message: '修改状态成功'
    ...
    approval (type,approvalStatus,tips) {
      ...
      if(ids.length == multipleSelection.length) {
        SERVICES.LOGISTICS_BASE_DATA.approveWithConsignee(ids,type).then(res => {
          if(res.status == 200) {
            this.$message({
                type: "success",
                message: '提交操作成功'
    ...
    delete (btn, row) {
      ...
      if (ids.length == multipleSelection.length) {
          this.$confirm(
            ...
            SERVICES.LOGISTICS_BASE_DATA.deleteRegistrationConsignee(ids).then((res) => {
              if(res.status == 200) {
                this.$message({
                    type: "success",
