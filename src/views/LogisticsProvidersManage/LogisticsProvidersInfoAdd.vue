...
<template>
  <div>
    ...
    <!-- 头部button End-->
    <!-- 头部Form Start-->
    <dynamic-form ref="headForm" :mate="mate.mateFormerHeader" :rules="headerFormRules" @events="handleEvents" @isDirty="dirtyChange"></dynamic-form>
    <!-- 头部Form End-->
  </div>
</template>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      headerFormRules: {
        // 物流商类型
        providerType: [{ required: true, trigger: "change", disabled: false, validator:changeSelect}],
        // 物流商名称
        providerName: [{ required: true, trigger: "blur", disabled: false, message:"此项不能为空"}],
        // 有效期开始
        validityStart: [{ required: true, trigger: "change", disabled: false, message:"此项不能为空"}],
        // 有效期结束
        validityEnd: [{ trigger: "change", disabled: false, validator:checkDate}],
        // 交易币种
        currency: [{ required: true, trigger: "change", disabled: false, message:"此项不能为空"}],
        // 创建人
  ...
  mounted() {
    // 创建日期默认值，默认为当天
    let date = this.mate.mateFormerHeader.values.createAt || '';
    this.mate.mateFormerHeader.values.createAt = date || dateFormatter("YYYY-MM-DD HH:mm:ss");
    // 创建人默认获取当前用户
  ...
  methods: {
     ...
     */
    backToList () {
      this.$root.monitor({ url: LOGISTIC_BASIC_DATA_API.LOGISTICS_PROVIDERS_INFO_LIST });
    },
    /**
     * 上传附件列表
     * @param void
