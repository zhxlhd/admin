...
export default {
  ...
  data() {
    return {
      ...
      headerFormRules: {
        // 合同类型编号
        contractTypeCode: [{ required: true, trigger: "blur", disabled: false, message:"此项不能为空"}],
        // 合同类型名称
        contractTypeName: [{ required: true, trigger: "blur", disabled: false, message:"此项不能为空"}]
      },
      headerFormEventConfig: {
        contractTypeCode: { blur: true }
      }
    };
  },
  created() {
    ...
    if(mateData && mateData.id && mateData.id !== '') {
      ...
      // 合同类型编号 修改的时候不允许修改，以保证合同类型编号唯一
      this.headerFormRules.contractTypeCode[0].disabled = true;
    }
  },
  methods: {
    /** 头部按钮事件
    ...
    handleEvents(event, val, name) {
      const eventControl = {
        blur: (val, name) => this.handleContractTypeCodeBlur(val, name),
        default: () => { return }
      }
      return (eventControl[event] || eventControl.default)(val, name);
    },
    /** blur 事件 Code 合同类型编码 校验是否唯一
     * @param {Any} val 控件的值
     * @param {String} name 控件的 name
     * @return void
     */
    handleContractTypeCodeBlur(val, name) {
      if (val !== '') {
        SERVICES.CONTRACT_MANAGE.checkContractTypeCode(val.trim()).then(res => {
          if(!res.data.status){
            this.mate.mateFormerHeader.values[name] = '';
            this.$message({
              type: "warning",
              message: "此合同类型编码已存在，请重新录入！"
            });
          }
        });
      }
