  ...
  methods: {
    ...
    handleButtons (btn, row, selection) {
      ...
      const btnEvents = {
        ...
        loseEfficacy: (btn, row) => this.loseEfficacy(btn, row),
        default: () => { return }
      }
    ...
    loseEfficacy (btn, row) {
      ...
      if(ids.length === 1) {
        SERVICES.CONTRACT_MANAGE.loseContractEfficacy(ids).then(res =>{
          if(res.status) {
            this.$message({
              type: "success",
              message: "操作成功"
    ...
    delete (btn, row) {
      ...
      if (ids.length >0 ) {
          this.$confirm(
            ...
            SERVICES.CONTRACT_MANAGE.deleteContractType(ids).then((res) => {
              if(res.status == 200) {
                this.$message({
                    type: "success",
