<!-- created by hezhiqiang on 2020/01/16 -->
<!-- 合同变更审批 -->
<template>
  <div>
  ...
  export default {
    ...
    methods: {
      ...
      handleRowClick (row, act) {
        let id = row.id;
        // 获取行数据
        this.getLines(id, 'contract_change');
      },
      /**
       * 头部按钮控制
       * 点击按钮触发对应方法
      ...
      handleButtons (btn, row, selection) {
        ...
        const btnEvents = {
          // 待审批状态(changeStatus=1)的可以同意(type=2)
          agree: () => this.approval('2',['1'],'TIPS_WARNING.FAILURE_TO_SUBMIT_FRO_APPROVAL'),
          // 待审批状态(changeStatus=1)的可以驳回(type=3)
          reject: () => this.approval('3',['1'],'TIPS_WARNING.CANNOT_BE_REJECTED'),
          default: () => { return }
        }
      ...
      approval (type,status,tips) {
        ...
        if(multipleSelection[0].changeStatus === 1) {
      ...
      handleConfirm() {
        ...
        this.$refs.reasonForm.validate(valid => {
          if (valid) {
            ...
            // 审批意见提交
            SERVICES.CONTRACT_MANAGE.contractChangeApproval({ businessId:ids.id[0], operationType:this.reasonData.reasonBtnType, businessType:'contract_change',approvalComment:reason }).then(res => {
              if(res.status == 200) {
                this.$message({
                  type: "success",
                  message: '提交操作成功'
       ...
       */
      reRenderList() {
        this.$root.monitor({ url: CONTRACT_MANAGE_API.CONTRACT_CHANGE_APPROVAL_LIST});
      },
    },
    watch: {
