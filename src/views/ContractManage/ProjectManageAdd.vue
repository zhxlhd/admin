  ...
  data() {
    var checkDate = (rule, value, callback) => {
      ...
      this.timerOut = setTimeout(() => {
        if(this.mate.mateFormerHeader.values['startDate'] && this.mate.mateFormerHeader.values['endDate'] && new Date(this.mate.mateFormerHeader.values['startDate']) > new Date(this.mate.mateFormerHeader.values['endDate'])){
          callback(new Error('项目结束日期不能小于起始日期'));
        }else{
          callback();
        }
      }, 300);
    ...
    return {
      ...
      headerFormRules: {
        // 项目编号
        projectCode: [{ required: true, trigger: "blur", disabled: false, message:"此项不能为空"}],
        // 项目名称
        projectName: [{ required: true, trigger: "blur", disabled: false, message:"此项不能为空"}],
         // 项目状态
        status: [{ required: true, type: 'number', trigger: "change", disabled: false, message:"此项不能为空"}],
         // 项目责任人
        charge: [{ required: true, trigger: "blur", disabled: false, message:"此项不能为空"}],
        // 项目开始日期
        startDate: [{ trigger: "change", disabled: false, validator:checkDate}],
        // 项目结束日期
        endDate: [{ trigger: "change", disabled: false, validator:checkDate}],
      },
      headerFormEventConfig: {
        projectCode: { blur: true }
      }
    };
  },
  created() {
    ...
    if(mateData && mateData.id && mateData.id !== '') {
      ...
      // 项目编号 修改的时候不允许修改，以保证项目编号唯一
      this.headerFormRules.projectCode[0].disabled = true;
    }
  },
  methods: {
    /** 头部按钮事件
    ...
    handleEvents(event, val, name) {
      const eventControl = {
        blur: (val, name) => this.handleProjectCodeBlur(val, name),
        default: () => { return }
      }
      return (eventControl[event] || eventControl.default)(val, name);
    },
    /** blur 事件 Code 项目编码 校验是否唯一
     * @param {Any} val 控件的值
     * @param {String} name 控件的 name
     * @return void
     */
    handleProjectCodeBlur(val, name) {
      if (val !== '') {
        SERVICES.CONTRACT_MANAGE.checkProjectCode(val.trim()).then(res => {
          if(!res.data.status){
            this.mate.mateFormerHeader.values[name] = '';
            this.$message({
              type: "warning",
              message: "此项目编码已存在，请重新录入！"
            });
          }
        });
      }
     ...
     */
    backToList () {
      this.$root.monitor({ url: CONTRACT_MANAGE_API.PROJECT_MANAGE_LIST });
    }
  }
};
</script>
