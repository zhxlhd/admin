  ...
  export default {
    ...
    methods: {
      ...
      handleRowClick (row, act) {
        let id = row.id;
        // 获取行数据
        this.getLines(id, 'contract_template');
      },
      /**
       * 重新渲染列表页
       * @param void
      ...
      handleConfirm() {
        ...
        this.$refs.reasonForm.validate(valid => {
          if (valid) {
            ...
            // 审批意见提交
            SERVICES.CONTRACT_MANAGE.approveWithContractTemplate({ businessId:ids.id[0], operationType:this.reasonData.reasonBtnType, businessType:'contract_template',approvalComment:reason }).then(res => {
              if(res.status == 200) {
                this.$message({
                  type: "success",
                  message: '提交操作成功'
