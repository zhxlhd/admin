...
export default {
  ...
  data() {
    var checkDate = (rule, value, callback) => {
      ...
      this.timerOut = setTimeout(() => {
        if(new Date(this.mate.mateFormerHeader.values['startDate']) > new Date(this.mate.mateFormerHeader.values['endDate'])){
    ...
    return {
      ...
      headerFormRules: {
        // 合同编号
        contractCode: [{ trigger: "blur" }],
        // 合同名称
        contractName: [{ required: true, trigger: "blur", disabled: false, message:"此项不能为空"}],
        // 合同类型
        ...
        // 相对人
        relativePerson: [{ required: true, trigger: "change", disabled: false, message:"此项不能为空"}],
        // 起始日期
        startDate: [{ required: true, trigger: "change", disabled: false, message:"此项不能为空"}],
        // 终止日期
        endDate: [{ required: true,trigger: "change", disabled: false, validator:checkDate}],
      },
  ...
  handleContractCodeBlur(val, name) {
    if (val !== '') {
      SERVICES.CONTRACT_MANAGE.checkContractCode(val.trim()).then(res => {
        if(!res.data.status){
          this.mate.mateFormerHeader.values[name] = '';
          this.$message({
            type: "warning",
            message: "此合同编码已存在，请重新录入！"
          });
        }
      });
    }
   ...
   */
  backToList () {
    this.$root.monitor({ url: CONTRACT_MANAGE_API.CONTRACT_DRAFT_LIST });
  },
  /**
   * 获取合同类型下拉列表
   * @param void
    ...
    SERVICES.CONTRACT_MANAGE.getProjectList().then(res => {
      if(res.data.length>0){
        ...
        if(data.length){
          ...
          // 项目名称保存到表单配置中
          this.headForm['projectId'].options = projectOptions;
        }
      }
    });
  }
