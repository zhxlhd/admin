...
export default {
  ...
  data(){
    return {
      ...
      uploadAttachmentParams: { rowId:'', serviceModel:'CONTRACT_MANAGE', getMethod:'getAttachMentListWithContractDraft', uploadMethod:'uploadAttachMent', deleteMethod:'deleteAttachMent', busType:'contract_draft' } // 上传附件属性配置
    }
  },
  props: {
    mate: Object
  ...
  methods: {
    ...
    handleButtons (btn, row, selection) {
      ...
      const btnEvents = {
        ...
        submitApprove: () => this.approval('2',['1'],'TIPS_WARNING.FAILURE_TO_SUBMIT_FRO_APPROVAL'),
    ...
    delete (btn, row) {
      ...
      if (ids.length == multipleSelection.length) {
        this.$confirm(
          ...
          SERVICES.CONTRACT_MANAGE.deleteContract(ids).then((res) => {
            if(res.status == 200) {
              this.$message({
                  type: "success",
    ...
    approval (type,status,tips) {
      ...
      if(ids.length>0) {
        SERVICES.CONTRACT_MANAGE.approveWithContract({ businessId:ids[0], operationType:type, businessType:'contract' }).then(res => {
          if(res.status == 200) {
            this.$message({
                type: "success",
                message: '提交操作成功'
