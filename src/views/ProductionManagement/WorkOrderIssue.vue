...
export default {
  ...
  methods: {
    ...
    handleStatus(status, ids, text) {
      SERVICES.WORK_ORDER_ISSUE.handleStatus(status, ids, text).then((res) => {
        if (!res.data.data){
          this.$message.error(`物料库存不足,请补充库存`);
        }
        this.reRenderList()
      });
    },
     ...
     */
    reRenderList() {
      this.$root.monitor({ url: PRODUCTION_MANAGEMENT_API.WORK_ORDER_ISSUE_MAIN_LIST });
    },
  },
  watch: {
