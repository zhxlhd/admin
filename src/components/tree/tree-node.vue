<template>
  <ul>
    <li v-for='item in nodeData' v-show="item.visible" :class="[(item.children && item.children.length > 0) ? '':'leaf']">
      ...
      <span @click="handleNode(item)" :class="{'node-selected':(item.checked && !options.showCheckbox) || item.searched }">{{item.name}}</span>
...
<style scoped>
  ...
  /*.halo-tree > ul > li + li::before {
    display: none
  }

  .halo-tree > ul > li + li:after {
    display: none
  }*/
  ...
  width:14px;
  height:14px;
  background-image: url("../../assets/images/treeOpen-1.png");
}

.tree-open{
  width:14px;
  height:14px;
  background-image: url("../../assets/images/treeClose-2.png");
}
.search{
  width:14px;
  height:14px;
